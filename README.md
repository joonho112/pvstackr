...
companions) so you can reuse it and replace only the part that differs.
All three are required for a reportable fit — an adapter that reports no
sampler diagnostics leaves the evidence incomplete and the fit is
blocked.
