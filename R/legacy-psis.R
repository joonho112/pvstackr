...
#'
#' `pv_migrate_legacy_psis_fit()` returns a `stack_psis` fit unchanged when it
#' passes the checks of the current version of pvstackr. Any other
...
#' their check, but no estimates, weights or draws. The fit that you pass is
#' not modified.
#'
#' @details
...
#' any numbers over: [get_estimates()] and [get_draws()] stop with an error
#' on it, and [get_target()] returns `NULL`. To get estimates, fit the model
#' again with [pv_fit_stack_psis()]. [get_diagnostics()], `print()` and
#' `summary()` work on the inspection object.
#'
...
#' (`hard_threshold`) and the thresholds, where present, recorded in the old
#' fit's diagnostics (`diagnostic_threshold`) and settings
#' (`control_threshold`), so an old fit is never judged against a threshold
#' above 0.7. `fallback_requested` is the old `fallback` setting (`"block"`,
#' `"warn"` or `"unknown"`). `get_diagnostics(x)$redaction` lists what was
#' removed.
#'
#' `pv_migrate_legacy_psis_fit()` stops with an error for a fit of another
#' method.
#'
#' @param fit A fit with `method = "stack_psis"`, made by the current or an
#'   earlier version of pvstackr, or an inspection object made by this
#'   function, which is checked and returned unchanged.
#' @param x,object An inspection object; for the `print()` method of a
#'   summary, the list returned by `summary()`.
#' @param ... Ignored.
#'
#' @returns `pv_migrate_legacy_psis_fit()` returns the fit itself when it
#'   passes the current checks, also when it is blocked. Otherwise it returns
#'   a list of class `pvstackr_legacy_psis_inspection` with the elements
#'   `method` (`"stack_psis"`), `inspection_only` (`TRUE`), `reportable`
#'   (`FALSE`), `reason_codes` (`"legacy_psis_inspection_only"`),
#'   `diagnostics` (`psis` and `redaction`; see Details), `source` (the
#'   class, format version and status of the old fit, `"unknown"` where they
#'   cannot be read), `schema_version` (the format version of the inspection
#'   object), `provenance` (a record of how it was made) and `warnings`.
#'
#'   `print()` returns its input invisibly; it shows the status of the old
#'   fit and `evidence_status`. `summary()` returns a list of class
#'   `summary.pvstackr_legacy_psis_inspection` with `method`, `status`
