...
**not** a claim that the stacked path is faster. The package reports no
timings and makes **no** efficiency or “faster” claim (the H5 guardrail;
A4 §7’s honesty triad). The single stacked fit can be larger and slower
per fit than one per-PV fit — it carries \\M\\ times the rows — so the
count is genuinely a statement about *number of fits*, not wall-clock
time.
