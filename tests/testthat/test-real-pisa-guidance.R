...
# The real-data guidance lives in the Applied track as
# `a5-real-pisa-guidance.Rmd` (formerly `real-pisa-guidance.Rmd`).
...
test_that("real PISA guidance records licensing and scope boundaries", {
  ...
  expect_match(guidance, "`pv_brr_target()` stops with an error when the formula", fixed = TRUE)
  expect_match(guidance, "control$center = \"target\"", fixed = TRUE)
  expect_match(guidance, "the center separation, which shows how far", fixed = TRUE)
  expect_match(guidance, "no fitting function[[:space:]]+uses it")
...
# The prebuilt vignette in inst/doc is a build product, so this check is
# render-gated like the live-render check below.
