...
test_that("frontmatter files exist and record scope", {
  ...
  # README.md is re-wrapped by pandoc, so phrases are matched on the text
  # with all runs of white space made single spaces.
  ...
  # What the package does, and the target.
  expect_match(readme_flat, "fits survey-weighted linear regression models", fixed = TRUE)
  expect_match(readme_flat, "reports estimates, standard errors and intervals for the fixed effects", fixed = TRUE)
  expect_match(readme_flat, "computes the design-based target", fixed = TRUE)
  expect_match(readme_flat, "balanced repeated replication with Fay", fixed = TRUE)
  expect_match(readme_flat, "the reported estimates, standard errors and degrees of freedom are those of the target", fixed = TRUE)
  expect_match(readme_flat, "`stack_direct` requires `center = \"target\"`", fixed = TRUE)
  expect_match(readme_flat, "the default in `pv_control()`", fixed = TRUE)
  expect_match(readme_flat, "supplies the calibrated draws", fixed = TRUE)
  expect_match(readme_flat, "diagnostics that compare the fit with the target", fixed = TRUE)
  # The other two methods.
  expect_match(readme_flat, "`per_pv` | combines one Bayesian fit per plausible value", fixed = TRUE)
  expect_match(readme_flat, "have no bundled engine and do not use the target", fixed = TRUE)
  expect_match(readme_flat, "variance within each plausible value is model-based", fixed = TRUE)
  expect_match(readme_flat, "the name and version of the program that computed them", fixed = TRUE)
  # Scope: fixed effects only.
  expect_match(readme_flat, "Only the fixed effects (the intercept and the slopes) are reported", fixed = TRUE)
  expect_match(readme_flat, "are neither calibrated nor reported", fixed = TRUE)
  # The example and the real-data sketch.
  expect_match(readme_flat, "An example on synthetic data", fixed = TRUE)
  expect_match(readme_flat, "pisa_tiny.csv", fixed = TRUE)
  expect_match(readme_flat, "pisa_tiny_stack_direct.rds", fixed = TRUE)
  expect_match(readme_flat, "no real PISA records", fixed = TRUE)
  expect_match(readme_flat, "not suitable for real inference", fixed = TRUE)
  expect_match(readme_flat, "detect_pisa_pv_columns", fixed = TRUE)
  expect_match(readme_flat, "pv_brr_target", fixed = TRUE)
  expect_match(readme_flat, "get_target", fixed = TRUE)
  expect_match(readme_flat, "get_diagnostics", fixed = TRUE)
  expect_match(readme_flat, "get_draws", fixed = TRUE)
  expect_match(readme_flat, "Fitting real data", fixed = TRUE)
  expect_match(readme_flat, "The code below is not run here: pvstackr includes no real data", fixed = TRUE)
  expect_match(readme_flat, "vignette(\"a5-real-pisa-guidance\"", fixed = TRUE)
  expect_match(readme_flat, "PV1MATH", fixed = TRUE)
  expect_match(readme_flat, "pv_suffix = \"MATH\"", fixed = TRUE)
  expect_match(readme_flat, "bare plausible-value names such as `PV1`", fixed = TRUE)
  expect_match(readme_flat, "N \\* M")
  expect_match(readme_flat, "M \\* \\(R \\+ 1\\)")
  # The companion methods preprint is cited by its DOI.
  expect_match(readme_flat, "https://doi.org/10.5281/zenodo.22407935", fixed = TRUE)
  expect_false(grepl("forthcoming", readme_flat, fixed = TRUE))
